const
    ...
    indexJSFile     = require('./webpack/index')(customConfig),
...
config.module.rules = [
    ...
    },
    {
        test: /\.jsx$|\.js$/,
        exclude: /(node_modules|vendor|bower_components)/,
        loaders: _v.NODE_ENV === 'development' ? ['react-hot-loader/webpack','babel-loader?presets[]=react-hmre'] : ['babel-loader']
    },
    //TEMPLATES (PUG)
    {
        test: /\.pug$/,
...
switch (_v.NODE_ENV) {
  ...
  case "development": {
      ...
      if(customConfig.requiresTemplate && !process.env.ELECTRON) {
          //WEB DEV MODE
      ...
      if(customConfig.enableRemoteDebugging && _v.NODE_ENV === "development") {
...
config.stats = funcs.getStats(_v.NODE_ENV);
